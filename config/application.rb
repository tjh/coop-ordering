...
require 'to_currency'
Dir[File.expand_path(File.join(Rails.root,'lib/core_extensions','*.rb'))].each {|f| require f}

Time::DATE_FORMATS[:pretty] = lambda { |time| time.strftime("%A, %B %e at %l:%M") + time.strftime("%p").downcase }
Time::DATE_FORMATS[:table] = lambda { |time| time.strftime("%b %d, %y at %l:%M") + time.strftime("%p").downcase }
